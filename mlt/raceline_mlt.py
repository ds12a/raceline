...
class MLTCollocation(PSCollocation):
    ...
    def iteration(self, t: np.ndarray, N: np.ndarray, warm_start: None | Trajectory = None):
        ...
        for k in range(K):
            ...
            U.append(self.opti.variable(N[k] + 1, self.n_u))
    ...
    @staticmethod
    def cost(q_1_dot, k_f=1e-3, k_b=1e-3):
    ...
    def sample_cost(self, traj: Trajectory, points: np.ndarray) -> tuple[np.ndarray, float]:
        ...
        # fig.add_traces(
        #     [
        #        go.Scatter(x=points*traj.length, y=v_lin),
        #        go.Scatter(x=points*traj.length, y=v_poly),
        #        go.Scatter(x=points*traj.length, y=fz_lin),
        #        go.Scatter(x=points*traj.length, y=fz_poly),
        #     ]
        # )
        # fig.show()
        ...
        return v_defect + fz_defect + 1e-6, np.trapezoid((v_defect + fz_defect), x=points) + 1e-6
